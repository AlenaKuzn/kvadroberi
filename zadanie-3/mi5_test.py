...
class LineFollowingNode(Node):
    ...
    def adjust_altitude(self):
        if self.current_pose is not None:
            # Decrease altitude by 0.01 meters
            target_altitude = self.current_pose.z - 0.01

            # Create a PoseStamped message to update the position
            pose = PoseStamped()
            pose.header.stamp = self.get_clock().now().to_msg()
            ...
            self.local_pos_pub.publish(pose)
            self.get_logger().info(f'Adjusting altitude to: {target_altitude}')
